...
func (c *Consumer) RenewLease(pid int, ttl time.Duration) bool {
	...
	l, ok := c.leases[pid]
	if !ok {
		l = model.Lease{Partition: pid}
	}
	// BUG(07): the holder check is dropped, so any consumer can renew a lease
	// it does not hold. An expired member's lease is silently taken over by
	// whoever calls RenewLease next, the old holder is never evicted from the
	// group, and the partition cannot migrate to an active member. The old
	// holder still believes it owns the partition, so two members end up
	// processing the same partition concurrently and the group's assignment
	// bookkeeping no longer matches who actually holds the partition.
	l.Holder = c.memberID
