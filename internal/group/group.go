...
func (c *Coordinator) Ack(group, member string) bool {
	...
	m.LastAck = c.now()
	// BUG(09c): acking a message also refreshes the heartbeat timestamp, so the
	// two liveness signals are conflated: the group can no longer tell a member
	// that is merely processing messages from one that is still heartbeating, and
	// the heartbeat timeout stops protecting idle-but-alive consumers.
	m.LastHeartbeat = c.now()
	return true
}
